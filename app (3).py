...
st.markdown(
    ...
    .feature-count-card .big {
        color: #1f2937;
        font-size: 1.22rem;
        font-weight: 800;
        line-height: 1.05;
    }
    .feature-count-card .small {
        color: #6f7a88;
        font-size: 0.72rem;
        margin-top: 0.35rem;
        line-height: 1.35;
    ...
    .about-card {
        ...
        border-radius: 13px;
        padding: 18px 20px;
        box-shadow: 0 2px 8px rgba(20, 33, 45, 0.04);
    }
    ...
    .about-card h3 { margin: 0 0 7px; font-size: 1rem; color:#27313d; }
    .about-card p { margin:0; color:#5f6b78; font-size:.86rem; line-height:1.55; }
    .about-card strong { color:#293542; }
    .consistency-note {
        background:#fff8e8;
        border:1px solid #f1d58a;
        border-left:4px solid #e0a72a;
        border-radius:10px;
        padding:12px 14px;
        color:#5f512d;
        font-size:.82rem;
...
if len(selected_features) != 33:
    st.warning(f"This model bundle contains {len(selected_features)} final features instead of the expected 33 from ABCCC (2).ipynb.")
...
with st.sidebar:
    ...
    st.markdown(
        f"""
        <div class="feature-count-card">
            <div class="big">33 / 33 Prediction Inputs</div>
            <div class="small">All final features from ABCCC (2).ipynb are included in the model input panel.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )

    with st.expander("View all 33 feature names", expanded=False):
    ...
    st.divider()
    st.markdown('<div class="feature-section-label"><span>Farm Profile</span><span class="feature-badge">4</span></div>', unsafe_allow_html=True)
    ...
    st.divider()
    st.markdown('<div class="feature-section-label"><span>Rainfall & Irrigation</span><span class="feature-badge">8</span></div>', unsafe_allow_html=True)
    ...
    st.divider()
    st.markdown('<div class="feature-section-label"><span>Temperature</span><span class="feature-badge">8</span></div>', unsafe_allow_html=True)
    temp_cols = [
    ...
    st.divider()
    st.markdown('<div class="feature-section-label"><span>Wind Speed</span><span class="feature-badge">4</span></div>', unsafe_allow_html=True)
    wind_speed_cols = [
    ...
    st.divider()
    st.markdown('<div class="feature-section-label"><span>Wind Direction</span><span class="feature-badge">4</span></div>', unsafe_allow_html=True)
    wind_dir_cols = [
    ...
    st.divider()
    st.markdown('<div class="feature-section-label"><span>Relative Humidity</span><span class="feature-badge">4</span></div>', unsafe_allow_html=True)
    humidity_cols = [
    ...
    st.divider()
    st.markdown('<div class="feature-section-label"><span>Production Indicator</span><span class="feature-badge">1</span></div>', unsafe_allow_html=True)
...
# ============================================================
# Main navigation — requested project flow
# ============================================================
main_tabs = st.tabs([
    "About",
    "Data Understanding",
...
# ============================================================
# 1. About — Business Understanding and project story
# ============================================================
with main_tabs[0]:
    st.markdown(
        """
        <div class="hero">
            <h1>About PaddyWise</h1>
            <p>Business problem, proposed machine-learning solution, modelling approach, improvement steps and final comparison results.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <div class="about-grid">
            <div class="about-card">
                <div class="about-kicker">01 · Problem</div>
                <h3>What is the problem?</h3>
                <p>Paddy production is affected by many agricultural and environmental conditions at the same time. Manually reviewing soil, rainfall, temperature, humidity, wind, nursery practice and other farm records is slow and can make consistent production planning difficult.</p>
            </div>
            <div class="about-card">
                <div class="about-kicker">02 · Solution</div>
                <h3>What solution are we proposing?</h3>
                <p>PaddyWise proposes a machine-learning classification dashboard that converts farm characteristics into one of four expected total paddy production levels: <strong>Low, Moderate, High, or Very High</strong>.</p>
            </div>
            <div class="about-card">
                <div class="about-kicker">03 · Models</div>
                <h3>What models were used?</h3>
                <p>The latest modelling notebook compares <strong>Logistic Regression</strong>, <strong>K-Nearest Neighbours (KNN)</strong>, <strong>Random Forest</strong>, and an <strong>Artificial Neural Network (ANN)</strong>.</p>
            </div>
            <div class="about-card">
                <div class="about-kicker">04 · Improvement</div>
                <h3>What was done to improve performance?</h3>
                <p>The workflow removes duplicate and highly redundant variables, creates a four-class target, performs a stratified train-test split, one-hot encodes categorical predictors, scales numerical predictors where required, and applies cross-validation plus hyperparameter tuning.</p>
            </div>
            <div class="about-card wide">
                <div class="about-kicker">05 · Results</div>
                <h3>What are the model comparison results?</h3>
                <p><strong>Logistic Regression</strong> achieved the best overall notebook result with <strong>97.44% accuracy</strong> and <strong>97.78% Macro F1</strong>. KNN achieved 95.21% accuracy / 95.49% Macro F1, Random Forest 95.38% / 95.14%, and ANN 89.74% / 90.41%.</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown('<div class="section-title">2.0 Business Understanding</div>', unsafe_allow_html=True)

    st.markdown("### 2.1 Company Background")
    st.write(
        "PaddyWise Agricultural Analytics Sdn. Bhd. is an agricultural technology firm that provides data analytics and decision-support services to paddy farmers, agricultural cooperatives, farm managers, and other rice-producing organisations. The project uses agricultural, cultivation, irrigation and weather information to support more structured, data-driven production planning."
    )

    st.markdown("### 2.2 Business Problem")
    st.write(
        "Paddy development and productivity depend on the combined effect of multiple factors such as soil type, rainfall, temperature, humidity, wind conditions, farm location and cultivation practices. Evaluating these factors separately may not provide a complete view of expected production, while manually analysing a large set of farm records is time-consuming and inconsistent."
    )

    st.markdown("### 2.3 Project Objective")
    st.write(
        "The objective is to develop a machine-learning classification system that predicts the expected level of total paddy production from a farm's agricultural, cultivation and environmental characteristics. Paddy yield is converted into four categories: Low (0–9,999 kg), Moderate (10,000–19,999 kg), High (20,000–29,999 kg), and Very High (30,000–40,000 kg)."
    )

    st.markdown("### 2.4 Significance and Potential Business Impact")
    st.write(
        "The proposed system can reduce the time required to assess agricultural records, improve consistency in farm-level recommendations, support production planning and resource allocation, and provide PaddyWise with a more structured analytical service for farmers, cooperatives and agricultural organisations."
    )

    st.markdown(
        """
        <div class="consistency-note">
        <strong>Project consistency note:</strong> the Business Understanding text supplied in the report mentions Decision Tree and says Trash (in bundles) should be excluded, while the latest <em>ABCCC (2).ipynb</em> model comparison uses ANN and the current 33-feature modelling set retains Trash. This dashboard follows the latest notebook for modelling and performance results. The report should be aligned with the final notebook before submission.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.caption("The 33 sidebar inputs remain active. This panel shows the prediction from the model selected in the sidebar.")
    ...
    clean_plot_layout(fig, height=340, margin=dict(l=20, r=70, t=20, b=45), legend=False)
...
# ============================================================
# 4. Model Performance — close to screenshot scorecard + CV + training tabs
# ============================================================
with main_tabs[3]:
    st.markdown(
        """
