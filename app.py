...
def load_data():
    ...
    raw["Paddy Yield Category"] = pd.cut(
        raw["Paddy yield(in Kg)"],
...
with main_tabs[1]:
    ...
    with du_tabs[0]:
        ...
        fig.update_layout(
            title=dict(text="Paddy Yield Class Distribution", x=0.5, font=dict(size=26)),
            xaxis_title="Paddy Yield Category",
            yaxis_title="Count",
            showlegend=False,
        )
        clean_plot_layout(fig, height=520, margin=dict(l=55, r=25, t=90, b=65), legend=False)
        st.plotly_chart(fig, use_container_width=True, config=PLOT_CONFIG)

        # Pie chart retained from the Data Understanding notebook
        st.markdown('<div class="section-title" style="margin-top:0.6rem;">Paddy Yield Category Share</div>', unsafe_allow_html=True)
        pie_fig = go.Figure(
            data=[
                go.Pie(
                    labels=dist["Yield Level"],
                    values=dist["Count"],
        ...
        pie_fig.update_layout(
            title=dict(
                text=f"Share of plots in each paddy yield category (n={len(raw_df):,})",
                x=0.5,
                font=dict(size=22),
            ),
            showlegend=True,
            ...
            margin=dict(l=60, r=60, t=85, b=95),
            height=560,
            paper_bgcolor="white",
            plot_bgcolor="white",
            font=dict(color="#252a35"),
        )
        st.plotly_chart(pie_fig, use_container_width=True, config=PLOT_CONFIG)
        st.caption("The pie chart follows the Data Understanding notebook and reports the percentage share of the four paddy-yield classes.")

    # Correlation map — compact 10x10, like the screenshot
    with du_tabs[1]:
        selected_numeric = [c for c in selected_features if c in raw_df.columns and pd.api.types.is_numeric_dtype(raw_df[c])]
        target = "Paddy yield(in Kg)"
        corr_all = raw_df[selected_numeric + [target]].corr(numeric_only=True)
        abs_target = corr_all[target].drop(target).abs().sort_values(ascending=False)
        top_predictors = abs_target.head(9).index.tolist()
        corr_display = raw_df[top_predictors + [target]].corr(numeric_only=True)
        ...
        fig.update_layout(
            title=dict(text="Correlation Map", x=0.5, font=dict(size=22)),
            coloraxis_colorbar=dict(title="", thickness=24, len=0.86),
        )
        fig.update_xaxes(tickangle=-90, title="")
        fig.update_yaxes(title="")
        clean_plot_layout(fig, height=690, margin=dict(l=185, r=55, t=75, b=190), legend=False)
        st.plotly_chart(fig, use_container_width=True, config=PLOT_CONFIG)
        st.caption("The dashboard shows the strongest non-redundant numerical predictors retained for modelling, together with the target, so the heatmap remains readable like the reference dashboard.")
...
with main_tabs[2]:
    ...
    st.markdown('<div class="section-title">Model Scorecard (Held-Out Test Set)</div>', unsafe_allow_html=True)
    ...
    numeric_high = ["Accuracy", "Precision", "Recall", "F1 Score", "AUC"]
    styled = (
        score.style
        .format({c: "{:.4f}" for c in score_cols if c != "Model"})
        .highlight_max(subset=numeric_high, color="#d9f0df")
    ...
    baseline_name = "Logistic Regression (Baseline)" if "Logistic Regression (Baseline)" in metrics_df["Model"].values else metrics_df.iloc[0]["Model"]
    st.markdown(
        f'<div class="champion-line"><b>Best held-out F1 model:</b> {MODEL_SHORT.get(best_f1_model, best_f1_model)}</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="champion-line"><b>Baseline Model:</b> {MODEL_SHORT.get(baseline_name, baseline_name)}</div>',
        unsafe_allow_html=True,
    )

    st.markdown('<div class="section-title">Performance Comparison</div>', unsafe_allow_html=True)
    perf_long = score.melt(
        id_vars="Model",
    ...
    clean_plot_layout(fig, height=480, margin=dict(l=35, r=20, t=35, b=105), legend=True)
    st.plotly_chart(fig, use_container_width=True, config=PLOT_CONFIG)
...
with main_tabs[3]:
    ...
    prep_rows = [
        ...
        ("5", "Create multiclass target", "Low: 0–9,999 kg; Moderate: 10,000–19,999 kg; High: 20,000–29,999 kg; Very High: 30,000–39,999 kg."),
